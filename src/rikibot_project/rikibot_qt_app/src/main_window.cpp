...
#include <QMessageBox>
#include <iostream>
#include "../include/rikibot_interface/main_window.hpp"

//add
//#include "/opt/ros/kinetic/include/nodelet/nodelet.h"
	...
	, qnode(argc,argv)
{
    ...
    //读取配置文件
    ReadSettings();
    setWindowIcon(QIcon(":/images/rikibot.png"));
    ...
    **********************/
    ...
    //imageView = new class ImageView(); // add
    //链接connect
    connections();

    // rqt_image_view::ImageView *imageView=NULL;
...
void MainWindow::initVideos()
{
   ...
   QSettings video_topic_setting("video_topic","rikibot_interface");
...
void MainWindow::connections()
{
    ...
    //add
    connect(ui.topics_combo_box, SIGNAL(currentIndexChanged(int)), this, SLOT(onTopicChanged(int)));
    connect(ui.topics_combo_box1, SIGNAL(currentIndexChanged(int)), this, SLOT(onTopicChanged1(int)));
...
void MainWindow::slot_set_return_point()
{
    ...
    //写入setting
    QSettings settings("return-position", "rikibot_interface");
...
void MainWindow::ReadSettings() {
    QSettings settings("Qt-Ros Package", "rikibot_interface");
    ...
    QSettings return_pos("return-position","rikibot_interface");
    ...
    //读取快捷指令的setting
    QSettings quick_setting("quick_setting","rikibot_interface");
...
QSet<QString> MainWindow::getTopics(const QSet<QString>& message_types, const QSet<QString>& message_sub_types, const QList<QString>& transports)
{
...
void MainWindow::updateTopicList()
{
  ...
  QString selected1 = ui.topics_combo_box1->currentText();
  // fill combo box
  ...
  for (QList<QString>::const_iterator it = topics.begin(); it != topics.end(); it++)
  {
    QString label(*it);
    label.replace(" ", "/");
    ui.topics_combo_box->addItem(label, QVariant(*it));
  }

  ui.topics_combo_box1->clear();
  for (QList<QString>::const_iterator it = topics.begin(); it != topics.end(); it++)
  {
    QString label(*it);
    label.replace(" ", "/");
    ui.topics_combo_box1->addItem(label, QVariant(*it));
  }
...
void MainWindow::selectTopic(const QString& topic)
{
  int index = ui.topics_combo_box->findText(topic);
  if (index == -1)
  {
    // add topic name to list if not yet in
    QString label(topic);
    label.replace(" ", "/");
    ui.topics_combo_box->addItem(label, QVariant(topic));
    index = ui.topics_combo_box->findText(topic);
  }
  ui.topics_combo_box->setCurrentIndex(index);
}

void MainWindow::selectTopic1(const QString& topic)
{
  int index = ui.topics_combo_box1->findText(topic);
  if (index == -1)
  {
    // add topic name to list if not yet in
    QString label(topic);
    label.replace(" ", "/");
    ui.topics_combo_box1->addItem(label, QVariant(topic));
    index = ui.topics_combo_box1->findText(topic);
  }
  ui.topics_combo_box1->setCurrentIndex(index);
}
...

{
  ...
  ROS_INFO_STREAM("aaaaaaaa");
  // reset image on topic change
  //ui.image_frame->setImage(QImage());
   ui.label_video0->setPixmap(QPixmap::fromImage(QImage()).scaled(ui.label_video0->width(),ui.label_video0->height()));
  ros::NodeHandle n;
  QStringList parts = ui.topics_combo_box->itemData(index).toString().split(" ");
  ...
  if (!topic.isEmpty())
  {
    ...
    image_transport::TransportHints hints(transport.toStdString());
     ROS_INFO_STREAM(transport.toStdString());
     //ROS_INFO_STREAM("ssss");
    try {
      ROS_INFO_STREAM("dadasdasdasd");
      subscriber_ = it.subscribe(topic.toStdString(), 1, &MainWindow::callbackImage, this, hints);
      //subscriber_ = it.subscribe(topic.toStdString(), 1, &MainWindow::callbackImage, this);
...
void MainWindow::onTopicChanged1(int index)

{
  ...
  // reset image on topic change
  //ui.image_frame->setImage(QImage());
   ui.label_video1->setPixmap(QPixmap::fromImage(QImage()).scaled(ui.label_video1->width(),ui.label_video1->height()));
  ros::NodeHandle n;
  QStringList parts = ui.topics_combo_box1->itemData(index).toString().split(" ");
  QString topic = parts.first();
  QString transport = parts.length() == 2 ? parts.last() : "raw";
  ...
  if (!topic.isEmpty())
  {
    ...
    try {
      ROS_INFO_STREAM("dadasdasdasd");
      subscriber1_ = it.subscribe(topic.toStdString(), 1, &MainWindow::callbackImage1, this, hints);
      //subscriber1_ = it.subscribe(topic.toStdString(), 1, &MainWindow::callbackImage, this);
      qDebug("ImageView::onTopicChanged() to topic '%s' with transport '%s'", topic.toStdString().c_str(), subscriber1_.getTransport().c_str());
      ROS_INFO_STREAM("ssss");
    } catch (image_transport::TransportLoadException& e) {
...
void MainWindow::callbackImage(const sensor_msgs::Image::ConstPtr& msg)
{
  ...
  //ROS_INFO_STREAM("kkkkkkkkkkkkkkkk");
...
void MainWindow::callbackImage1(const sensor_msgs::Image::ConstPtr& msg)
{
...
void rikibot_interface::MainWindow::on_pushButton_2_clicked()
{
