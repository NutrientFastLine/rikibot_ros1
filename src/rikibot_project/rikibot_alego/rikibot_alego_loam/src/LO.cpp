...
private:
  ...
  queue<cloud_msgs::cloud_infoConstPtr> seg_info_buf_;
...
public:
  ...
  void onInit()
  {
    ...
    sub_segmented_info_ = nh_.subscribe<cloud_msgs::cloud_info>("/seg_info", 10, boost::bind(&LO::segInfoHandler, this, _1));
  ...
  void mainLoop()
  {
    ...
    while (ros::ok())
    {
      ...
      if (!seg_cloud_buf_.empty() && !seg_info_buf_.empty() && !outlier_buf_.empty())
      {
        ...
        cloud_msgs::cloud_infoConstPtr seg_info = seg_info_buf_.front();
  ...
  void segInfoHandler(const cloud_msgs::cloud_infoConstPtr &msg)
  {
    m_buf_.lock();
